...
import com.rasgo.combiuser.databinding.ActivityMapTripBinding
import com.rasgo.combiuser.fragments.ModalBottomSheetTripInfo
...
class MapTripActivity : AppCompatActivity(), OnMapReadyCallback, Listener, DirectionUtil.DirectionCallBack {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.btnFinishTrip.setOnClickListener { finishTrip() }
    }
        ...
        listenerBooking = bookingProvider.getBooking().addSnapshotListener { document, e ->
            ...
            if (!isBookingLoaded) {
                ...
                googleMap?.moveCamera(CameraUpdateFactory.newCameraPosition(
                    CameraPosition.builder().target(originLatLng!!).zoom(20f).build()
                ))
                getLocationDriver()
    ...
    private fun addDriverMarker(position: LatLng) {
        markerDriver = googleMap?.addMarker(MarkerOptions().position(position).title("Tu conductor")
            .icon(BitmapDescriptorFactory.fromResource(R.drawable.icon_combi_pcyan)))
    }
