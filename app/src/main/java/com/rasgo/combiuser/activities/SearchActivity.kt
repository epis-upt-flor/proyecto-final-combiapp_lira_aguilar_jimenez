...
import com.rasgo.combiuser.databinding.ActivitySearchBinding
...
import com.rasgo.combiuser.providers.*
...
class SearchActivity : AppCompatActivity() {
    ...
    private fun sendNotification() {
        ...
        map.put("title", "BÚSQUEDA DE RUTA")
        map.put(
            "body",
            "Un cliente esta solicitando una combi a " +
            ...
            override fun onResponse(call: Call<FCMResponse>, response: Response<FCMResponse>) {
                if (response.body() != null) {
                    ...
                    if (response.body()!!.success == 1) {
                        Toast.makeText(this@SearchActivity, "Se envió la notificacion", Toast.LENGTH_LONG).show()
                    }
                    else {
        ...
        listenerBooking = bookingProvider.getBooking().addSnapshotListener { snapshot, e ->
            ...
            if (snapshot != null && snapshot.exists()) {
                ...
                if (booking?.status == "accept") {
                    Toast.makeText(this@SearchActivity, "Combi en camino", Toast.LENGTH_SHORT).show()
                    listenerBooking?.remove()
                    goToMapTrip()
                }
                else if (booking?.status == "cancel") {
                    Toast.makeText(this@SearchActivity, "Combi no está en ruta", Toast.LENGTH_SHORT).show()
                    listenerBooking?.remove()
                    goToMap()
                }
        ...
        geoProvider.getNearbyDrivers(originLatLng!!, radius).addGeoQueryEventListener(object: GeoQueryEventListener {
            ...
            override fun onKeyEntered(documentID: String, location: GeoPoint) {
                if (!isDriverFound) {
                    ...
                    binding.textViewSearch.text = "COMBI ENCONTRADA\nESPERANDO RESPUESTA"
            ...
            override fun onGeoQueryReady() { // TERMINA LA BUSQUEDA
                if (!isDriverFound) {
                    ...
                    if (radius > limitRadius) {
                        binding.textViewSearch.text = "NO SE ENCONTRO NINGUNA COMBI"
                        return
                    }
                    else {
                        getClosestDriver()
